...
test('harness declares vision, terminal, web, and computer tools', async () => {
  ...
  assert.match(ui, /id="balto-update-button"/)
  assert.match(ui, /invoke\('check_for_updates'\)/)
  assert.match(ui, /invoke\('install_update'\)/)
...
test('release claims use the installed app benchmark', async () => {
  ...
  assert.match(await read('scripts/verify-release.sh'), /for attempt in 1 2 3/)
  assert.match(await read('scripts/verify-release.sh'), /Internal Xprotect Error/)
  assert.match(await read('scripts/verify-release.sh'), /exit "\$syspolicy_status"/)
